            ...
            try
            {
                ...
                using (galadbEntities context = new galadbEntities())
                {
                    ...
                    if (LMaxEvt != null && LMaxEvt.Count != 0)
                        MaxEvt = LMaxEvt.FirstOrDefault().NUMEVENEMENT ;

                    if (_lstEvt.Count != 0)
                    {
                        if (_lstEvt.Max(t => t.NUMEVENEMENT) == MaxEvt)
                            ReturneValu = Galatee.Tools.Utility.ConvertEntity<CsEvenement, EVENEMENT>(_lstEvt.FirstOrDefault(t => t.NUMEVENEMENT == MaxEvt));
                            //EVENEMENT lev = _lstEvt.FirstOrDefault(t => t.NUMEVENEMENT == MaxEvt);
                            //if (IsLotIsole(lev.LOTRI))
                            //    ReturneValu = "2";
                            //if (!IsLotIsole(lev.LOTRI))
                            //    ReturneValu = "1";
                    }
