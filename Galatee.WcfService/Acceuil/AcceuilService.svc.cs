        ...
        public  List<CsForfait> ChargerForfait()
    {
        ...
        public CsAg RetourneAdresse(int fk_idcentre, string Centre, string Client, string ordre)
        {
            try
            {
                ...
                DBAccueil db = new DBAccueil();
                return db.RetourneAdresse(fk_idcentre, Centre, Client, ordre);
            }
            catch (Exception ex)
            {
                ErrorManager.WriteInLogFile(this, ex.Message);
        ...
        public List<ObjELEMENTDEVIS  > SelectAllMateriel()
        {
            try
            {
                ...
                return new DBAccueil().RetourneMaterielDevis();
            }
            catch (Exception ex)
            {
                ErrorManager.LogException(this, ex);
        ...
        public string ValidationReception(List<CsDetailAffectationScelle> ListeScelle, string MatAgent, int idetapeActuelle, string numdem)
        {
            ...
            {
                DBScelle db = new DBScelle();
                return db.ValidationReception(ListeScelle, MatAgent, idetapeActuelle, numdem);
            }
            catch (Exception ex)
            {
                ErrorManager.WriteInLogFile(this, ex.Message);
        ...
        {
            CsDemandeBase lademade = DBDEVIS.GetDevisByNumIdDevis(pIdDemandeDevis);
            return new DB_WORKFLOW().RecupererInfoDemandeParCodeTDem(lademade);
        }
        ...
        public List<CsEvenement > RetourneDernierEvtFacture(CsDemandeBase laDemande)
        {
            if (laDemande.PRODUIT != Enumere.Prepaye)
            {
                ...
                if (leAbon != null && leAbon.Count != 0)
                {
                    ...
                    return  new DbFacturation().RetourneEvenement(leClient);
                }
            }
            return null;
        }
        ...
        public List<CsEvenement> RetourneEvenementClient(CsClient LeClient)
        {
            try
            {
                ...
                if (LeClient.PK_ID == 0)
                {
                    leAbon = new DBAccueil().RetourneAbon(LeClient.FK_IDCENTRE.Value, LeClient.CENTRE, LeClient.REFCLIENT, LeClient.ORDRE, LeClient.PRODUIT);
                ...
                else
                {
                    ...
                    leAbon = new DBAccueil().Select_Abon(int.Parse(LeClient.FK_IDCENTRE.ToString()), LeClient.CENTRE, LeClient.REFCLIENT, LeClient.ORDRE);
                    if (leAbon != null && !string.IsNullOrEmpty(leAbon.CLIENT))
                        LeClient.FK_IDABON = leAbon.PK_ID;
