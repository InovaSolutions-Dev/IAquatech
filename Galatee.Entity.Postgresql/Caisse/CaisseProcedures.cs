       ...
       public static decimal ? RetourneEncaissementDate(CsHabilitationCaisse laCaisse)
       {
           try
           {
               using (galadbEntities context = new galadbEntities())
               {
                   ...
                   List<TRANSCAISSE> transcaisse = context.TRANSCAISSE.Where(p => p.FK_IDHABILITATIONCAISSE == laCaisse.PK_ID && p.MODEREG == Enumere.ModePayementEspece &&  (p.ISDEMANDEANNULATION == false || p.ISDEMANDEANNULATION == null) && p.DC == Enumere.Debit  ).ToList();
