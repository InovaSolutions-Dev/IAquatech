...
namespace Galatee.DataAccess
{
    ...
    public class DB_WORKFLOW : Galatee.DataAccess.Parametrage.DbBase
    {
        ...
        public List<CsGroupeValidation> SelectAllGroupeValidationSpecifique(int TypeGroupe)
        {
            try
            {
                List<CsGroupeValidation> lstGV = Entities.GetEntityListFromQuery<CsGroupeValidation>(ParamProcedure.LISTE_WKF_GROUPES_VALIDATION());
                if (lstGV != null)
        ...
        {
            return Entities.GetEntityListFromQuery<CsRHabilitationGrouveValidation>(ParamProcedure.LISTE_WKF_HABILITATION_GROUPE_VALIDATION(GrpPKID));
        }
    ...
    ref CsDemandeWorkflow dmd)
        ...
        public Dictionary<CsCopieDmdCircuit, CsCopieDmdConditionBranchement> SelectAllCircuitEtapeDemandeWorkflow(string codeDemande)
        {
            try
            {
                ...
                var lsGroupValidationWKF = Entities.GetEntityListFromQuery<CsGroupeValidation>(ParamProcedure.LISTE_WKF_GROUPES_VALIDATION());
                var lsEtapeWKF = Entities.GetEntityListFromQuery<CsEtape >(ParamProcedure.LISTE_WKF_ETAPES());
                //Ensuite les conditions s'il en existe
                Dictionary<CsCopieDmdCircuit, CsCopieDmdConditionBranchement> TheDatas = new Dictionary<CsCopieDmdCircuit, CsCopieDmdConditionBranchement>();
                foreach (CsCopieDmdCircuit aff in lsEtapesAffWKF)
        ...
        public List<CsVwJournalDemande> SelectAllDemandeUser(List<int> lstCentreId, List<Guid> lstIdDemande, int Fk_idetape, string Matricule)
        {
            try
            {
                ...
                List<CsVwJournalDemande> lsDemandeWfk = RetourneDemandeWkfEtape(lstCentreId, Fk_idetape, Matricule);
        ...
        public List<CsCommentaireRejet> SelectCommentaireRejet(string  NumDemande)
        {
            try
            {
                return Entities.GetEntityListFromQuery<CsCommentaireRejet>(ParamProcedure.LISTE_COMMENTAIRE_REJET(NumDemande))
                    .ToList();
            }
            catch (Exception ex)
            {
        ...
        {
            bool ilAppartient = true;
            galadbEntities dbEntities = new galadbEntities();
            ilAppartient = dbEntities.RHABILITATIONGROUPEVALIDATION.Where(g => g.FK_IDADMUTILISATEUR == UserId)
                .Count() > 0;
            dbEntities.Dispose();
            return ilAppartient;
        }
        ...
        public List<CsVwJournalDemande> RetourneDemandeWkfEtape(List<int> lstCentreId, int Fk_idetape, string Matricule)
        {
            cn = new SqlConnection(ConnectionString);
