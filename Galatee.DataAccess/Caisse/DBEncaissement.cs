...
namespace Galatee.DataAccess
{
    public class DBEncaissement
    {
        ...
        public List<CsClient> TestClientExist(string centre,string client,string ordre)
        {
            ...
            {
                //rows = FillClient(reader,centre ,client ,ordre );
                DataTable dt = Galatee.Entity.Model.CaisseProcedures.RetourneClient(centre,client,ordre);
                List<CsClient> _Lesclient = Entities.GetEntityListFromQuery <CsClient>(dt);
                //_Lesclient.ForEach(t => t.SOLDE = Galatee.Entity.Model.FonctionCaisse.RetourneSoldeClient(t.FK_IDCENTRE ,t.CENTRE ,t.REFCLIENT ,t.ORDRE ));
        ...
        public List<CsLclient > RetourneListeRecuDeCaisse(int Caisse)
        {
            try
            {
                DataTable dt = CaisseProcedures.RetourneListeRecuDeCaisse(Caisse);
                return Entities.GetEntityListFromQuery<CsLclient>(dt);
            }
            catch (Exception ex)
            {
       ...
       public bool? RetirerDemandeAnnulationEncaissement(List<CsLclient> ListFactureAnnule)
        ...
        public bool ReverserCaisse(List<CsReversementCaisse> laCaisseHAbil)
        {
            try
            {
                ...
                return Galatee.Entity.Model.CaisseProcedures.ReverserCaisse(laCaisseHAbil);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public CsHabilitationCaisse HabiliterCaisse(CsHabilitationCaisse laCaisseHAbil)
        {
            try
            {
                return  Galatee.Entity.Model.CaisseProcedures.HabiliterCaisse(laCaisseHAbil);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public List<CsHabilitationCaisse> RetourneHabileCaisseNonReversement(CsHabilitationCaisse laCaisse)
        {
            try
            {
                DataTable dt = Galatee.Entity.Model.CaisseProcedures.RetourneHabileCaisseNonReversement(laCaisse);
                List<CsHabilitationCaisse> lstHabil =  Entities.GetEntityListFromQuery<CsHabilitationCaisse>(dt);
                foreach (var item in lstHabil)
                {
                    item.MONTANTENCAISSE = RetourneEncaissementEspeceDate(item);
                    item.ECART = item.MONTANTENCAISSE - (item.MONTANTREVERSER == null ? 0 : item.MONTANTREVERSER); 
                }
                return lstHabil.Where(t => t.ECART > 0).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public CsHabilitationCaisse  RetourneHabileCaisseReversement(CsHabilitationCaisse laCaisse)
        {
            try
            {
                DataTable dt = Galatee.Entity.Model.CaisseProcedures.RetourneHabileCaisseReversement(laCaisse);
                List<CsReversementCaisse> lstHabil = Entities.GetEntityListFromQuery<CsReversementCaisse>(dt);
                if (lstHabil != null && lstHabil.Count == 1)
                    laCaisse.MONTANTREVERSER  = lstHabil.First().MONTANT ;
                else if (lstHabil != null && lstHabil.Count > 1)
                    laCaisse.MONTANTREVERSER = lstHabil.Sum(p => p.MONTANT );
                   
                laCaisse.MONTANTENCAISSE = RetourneEncaissementEspeceDate(laCaisse);
                laCaisse.ECART = laCaisse.MONTANTENCAISSE -  (laCaisse.MONTANTREVERSER == null ? 0:laCaisse.MONTANTREVERSER ) ;
                return laCaisse;
            }
            catch (Exception ex)
            {
        ...
        public string VerifieEtatCaisse(string matricule, int? Fk_IdCaisse)
        {
            ...
            try
            {
                ...
                switch (_resultatRequete)
                {
                    ...
                    case 5:
                        return Enumere.EtatDeCaisseAutreUserSurCaisse;
        ...
        #region ADO.net
            ...
            public bool EncaissementFactureSpx(List<CsLclient> LstDesReglementAInserer)
            {
                ...
                {
                    int result = 0;
                    foreach (CsLclient st in LstDesReglementAInserer)
                    {
                        if (st.NDOC == "TIMBRE" && st.MONTANT == 0)
                            continue;
                        if (result > 0 && st.FK_IDLCLIENT == 0)
                            st.FK_IDLCLIENT = result;
                        result = InsererPaiementFactureSpx(st, laCommande);
                    }
...
/*
            public bool EncaissementDemandeSpx(List<CsLclient> LstDesReglementAInserer)
            {
                ...
                try
                {
                    ...
                    UpdateAcquitsPX(LstDesReglementAInserer[0].FK_IDHABILITATIONCAISSE.Value, LstDesReglementAInserer[0].MATRICULE, Convert.ToInt16(LstDesReglementAInserer[0].ACQUIT), laCommande);
...
*/
            ...
            public bool EncaissementDemandeSpx(List<CsLclient> LstDesReglementAInserer)
            {
                ...
                try
                {
                    foreach (CsLclient FactureAregle in LstDesReglementAInserer)
                    {
                        if (FactureAregle.NDOC == "TIMBRE" && FactureAregle.MONTANT == 0)
                            continue;
...
*/
        ...
        public int InsererPaiementFactureSpx(CsLclient LstDesReglementAInserer, SqlCommand cmds)
            {
                ...
                SqlParameter outResult = new SqlParameter("@IDFACTURE", SqlDbType.VarChar, int.MaxValue) { Direction = ParameterDirection.Output };
                cmds.Parameters.Add(outResult);
                string id = string.Empty;
                ...
                try
                {
                    ...
                    if (LstDesReglementAInserer.FK_IDLCLIENT == 0)
                        result = int.Parse(id);
