...
namespace Galatee.DataAccess
{
     public class DBUserHabilitation : Galatee.DataAccess.Parametrage.DbBase
    {
        ...
        {
            //CsProgramMenu prog;
            //cmd.CommandText = "spx_ADM_Program_SelectAll";
           ...
           // try
            //{
                ...
                try
                {
                    ...
                    return Entities.GetEntityListFromQuery<CsProgramMenu>(dt).ToList();
                }
                catch (Exception ex)
                {
                    throw ex;
                ...
                //while (reader.Read())
                //{
                //   // prog = new CsProgramMenu();
                //   // if (reader.GetValue(0).ToString().Trim() != string.Empty)
                //   //     prog.IdGroupProgram = reader.GetValue(0).ToString().Trim();
                //   // if (reader.GetValue(1).ToString().Trim() != string.Empty)
                //   //     prog.IdProgram = reader.GetValue(1).ToString().Trim();
                //   // prog.ProgName = reader.GetValue(2).ToString().Trim();
                //   // prog.MainMenuID = reader.GetValue(3).ToString().Trim();
                //   //// prog.MenuID = reader.GetValue(4).ToString().Trim();
                //   // prog.ModuleName = reader.GetValue(5).ToString().Trim();
                //   // prog.MenuText = reader.GetValue(6).ToString().Trim();
                //   // programmes.Add(prog);
                //}
                //reader.Close();
