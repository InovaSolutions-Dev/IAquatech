    ...
    public class DBMoratoires
    {
        ...
        private void MiseAjourLclientCampagneDetailCampage(List<CsAvisDeCoupureClient> lstAvis, CsAvisCoupureEdition aviscoupure)
        {
            try
            {
                ...
                CAMPAGNE camp = RetourneCampagne(idcoupure, aviscoupure);
                camp.DETAILCAMPAGNE = RetourneDetailCampagne(lstAvis, idcoupure);

                foreach (DETAILCAMPAGNE item in camp.DETAILCAMPAGNE)
                {
        ...
        private CAMPAGNE RetourneCampagne(string idcoupure, CsAvisCoupureEdition coupure)
        {
            try
            {
                ...
                camp.FK_IDMATRICULE = contextInter.ADMUTILISATEUR.First(c => c.MATRICULE == camp.MATRICULEPIA).PK_ID;
        ...
        private List<DETAILCAMPAGNE> RetourneDetailCampagne(List<CsAvisDeCoupureClient> clientAvisCampagne, string idcoupure)
        {
            try
            {
                List<DETAILCAMPAGNE> lstDetailCampagne = new List<DETAILCAMPAGNE>();
                lstDetailCampagne = Entities.ConvertObject<DETAILCAMPAGNE, CsAvisDeCoupureClient>(clientAvisCampagne);
             
                //foreach (CsClient coupure in clientAvisCampagne)
                //{
                //    DETAILCAMPAGNE camp = new DETAILCAMPAGNE();
                //    camp.IDCOUPURE = idcoupure;
                //    camp.CENTRE = coupure.CENTRE;
                //    camp.CLIENT = coupure.REFCLIENT;
                //    camp.ORDRE = coupure.ORDRE;
                //    camp.ORDTOUR = coupure.ORDTOUR;
                //    camp.TOURNEE = coupure.TOURNEE;
                //    camp.CATEGORIECLIENT = coupure.CATEGORIE ;
                //    camp.SOLDEDUE = coupure.SOLDEDUE == null ? 0 : coupure.SOLDEDUE.Value;
                //    camp.BILLSDUE = coupure.TOTALFACUTREDUE == null ? 0 : coupure.TOTALFACUTREDUE.Value;
                //    camp.SOLDECLIENT = coupure.SOLDE == null ? 0 : coupure.SOLDE.Value;
                //    camp.DATECREATION = DateTime.Now.Date;
                //    camp.USERCREATION = coupure.MATRICULE;
                //    camp.COMPTEUR = coupure.COMPTEUR;
                //    camp.CATEGORIECLIENT  = coupure.CATEGORIE ;

                //    camp.DATEMODIFICATION = DateTime.Now.Date;
                //    camp.USERMODIFICATION = coupure.MATRICULE;
                //    camp.FK_IDCENTRE = coupure.FK_IDCENTRE.Value;
                //    camp.FK_IDCATEGORIECLIENT = coupure.FK_IDCATEGORIE.Value;
                //    camp.FK_IDTOURNEE = coupure.FK_IDTOURNEE;
                //    camp.FK_IDCLIENT = coupure.PK_ID;
                //    lstDetailCampagne.Add(camp);
                //}
        ...
        */
        ...
        private DataTable CreationLotCampagne(int idtournee, int idcategerie, int? idCodeConsomateur,
                                              ...
                                              DateTime? Exigibilite, int? NombreMaximunFacture, decimal? SoldeMinimum, string Periode, int? NombreTotalClient, int? IdPia, string matricule)
        {
            ...
            cmd.Parameters.Add("@idCodeconso", SqlDbType.Int).Value = idCodeConsomateur;
            cmd.Parameters.Add("@idTournee", SqlDbType.Int).Value = idtournee;
            cmd.Parameters.Add("@idCategorie", SqlDbType.Int).Value = idcategerie;
        ...
        #region Precontentieux
            ...
            public string GetNumeroFacture()
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "SPX_ACC_GET_NUMEROFACTURE";
                ...
                try
                {
                    ...
                    while (reader.Read())
                        resultat = (Convert.IsDBNull(reader["FACTURE"])) ? string.Empty : (System.String)reader["FACTURE"];
